class Question:
    def __init__(self, id, question, answer, link):
        self.id = id
...
questions_texts = ["Як оформити ВПО?", "Грошова допомога для ВПО", "Що таке ВПОHelpBot?", "Бот для волонтерів"]
answers_texts = [
    ...
    "\t🤖 ВПОHelpBot -- сервіc створений для допомоги ВПО та волонтерам, розроблений студентами ЛНУ Івана Франка. Це загальна база даних, яка збирає інформацію про волонтерські події та допомогу ВПО в одному зручному місці.\n "
...
Link = [
    'https://bf.diia.gov.ua/articles/yak-zareiestruvatys-vpo-otrymannia-dovidky-instruktsiia',
...
questions_list = {i: Question(i, q, a, l) for i, (q, a, l) in enumerate(zip(questions_texts, answers_texts, Link))}
